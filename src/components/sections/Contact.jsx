...
export default function Contact() {
  ...
  return (
    <section id="contact" style={{ position: 'relative', zIndex: 1 }} className="section-pad">
      ...
      <div className="contact-grid">
            ...
            {[
              ...
              item.href ? (
                <a key={i} href={item.href}
                  ...
                  style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', padding: '16px 0', borderBottom: '1px solid var(--border)', textDecoration: 'none', transition: 'padding-left 0.15s linear' }}
        ...
        {/* ── Form ────────────────────────────────── */}
        <div>
          <div style={{ border: '1px solid var(--border)', padding: '32px', backgroundColor: 'var(--paper)', position: 'relative' }}>
            <span className="corner-marker tl" /><span className="corner-marker tr" />
            <span className="corner-marker bl" /><span className="corner-marker br" />
            ...
            {status === 'success' ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '280px', gap: '16px' }}>
                ...
                <button onClick={() => setStatus('idle')} className="btn-ghost" style={{ marginTop: '8px', fontSize: '10px', padding: '6px 16px' }}>
                  Send Another
                </button>
              </div>
            ) : (
              <form ref={formRef} onSubmit={handleSubmit}>
                <div style={{ borderBottom: '1px solid var(--border)', paddingBottom: '16px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px', marginBottom: '16px' }}>
                  <div>
                    ...
                    <input className="form-input" type="text" name="from_name" placeholder="Jane Smith" value={form.from_name} onChange={handleChange} required />
                  </div>
                  <div>
                    <label className="form-label">Email</label>
                    <input className="form-input" type="email" name="from_email" placeholder="jane@company.com" value={form.from_email} onChange={handleChange} required />
                  </div>
                </div>

                <div style={{ marginBottom: '16px' }}>
                  <label className="form-label">Opportunity Type</label>
                  <input className="form-input" type="text" name="project_type" placeholder="Internship, freelance, research..." value={form.project_type} onChange={handleChange} />
                </div>

                <div style={{ marginBottom: '24px' }}>
                  <label className="form-label">Message</label>
                  <textarea className="form-input" name="message" rows="5" placeholder="Tell me more..." value={form.message} onChange={handleChange} required style={{ resize: 'vertical' }} />
                </div>

                {status === 'error' && (
                  <div style={{ marginBottom: '16px', padding: '10px 14px', border: '1px solid var(--coral)', backgroundColor: 'rgba(255,140,105,0.08)', fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--coral)', letterSpacing: '0.06em' }}>
                    ⚠ Failed to send — please email {PERSONAL.email} directly.
                  </div>
                )}

                <button type="submit" className="btn-solid" disabled={status === 'sending'}
                  style={{ width: '100%', justifyContent: 'center', padding: '12px', opacity: status === 'sending' ? 0.6 : 1, cursor: status === 'sending' ? 'not-allowed' : 'pointer' }}>
