...
import { SKILL_CATEGORIES, EXPERIENCE, LEADERSHIP } from '../../data/resume';

function SkillCell({ category }) {
  return (
    <div className="bento-cell">
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '24px' }}>
        <div style={{ width: '3px', height: '18px', backgroundColor: category.accent }} />
      ...
      <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: '700', letterSpacing: '-0.02em', marginBottom: '20px' }}>
        {category.label}
      </h3>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ height: '2px', backgroundColor: 'rgba(26,60,43,0.1)', width: '100%' }}>
    ...
    <section id="skills" style={{ position: 'relative', zIndex: 1, borderBottom: '1px solid var(--border)' }}>
      ...
      <div style={{ borderTop: '1px solid var(--border)' }}>
        <div style={{ padding: '48px 48px 24px' }}>
          <div className="text-section-label" style={{ marginBottom: '32px' }}>Professional Experience</div>
        </div>
              ...
              <div>
                ...
                {exp.current && (
                  <div style={{
                    ...
                    padding: '3px 8px', border: '1px solid var(--mint)',
                  }}>
                    <div style={{ width: '5px', height: '5px', backgroundColor: 'var(--mint)' }} />
      ...
      {/* Leadership */}
      <div style={{ borderTop: '1px solid var(--border)', padding: '48px' }}>
        <div className="text-section-label" style={{ marginBottom: '24px' }}>Leadership & Organizations</div>
        <div style={{
          display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)',
          gap: '1px', background: 'var(--grid)',
        }}>
          {LEADERSHIP.map((l, i) => (
            <div key={i} style={{ background: 'var(--paper)', padding: '20px' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: '600', marginBottom: '4px' }}>{l.role}</div>
              <div style={{ fontFamily: 'var(--font-body)', fontSize: '12px', opacity: 0.65, marginBottom: '8px' }}>{l.org}</div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', opacity: 0.4, letterSpacing: '0.08em' }}>{l.period}</div>
            </div>
          ))}
        </div>
      </div>
