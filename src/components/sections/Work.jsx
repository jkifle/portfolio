...
function ProjectCard({ project }) {
  return (
    <div className="bento-cell" style={{ position: 'relative', minHeight: '320px' }}>
      {/* Top label row */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '3px', height: '18px', backgroundColor: project.accent }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', opacity: 0.5 }}>
            {project.id}
          </span>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.1em', opacity: 0.4 }}>
            {project.year}
          </span>
      ...
      <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '22px', fontWeight: '700', letterSpacing: '-0.02em', marginBottom: '10px', lineHeight: '1.1' }}>
        {project.title}
      </h3>
      ...
      <div style={{ border: '1px solid var(--border)', padding: '14px', backgroundColor: 'rgba(26,60,43,0.03)', marginBottom: '20px' }}>
        {project.mockup.lines.map((line, i) => (
          <div key={i} style={{
            ...
            marginBottom: i < project.mockup.lines.length - 1 ? '8px' : '0',
          }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '20px' }}>
        {project.tech.map(t => (
          <span key={t} style={{
            padding: '3px 8px', border: `1px solid ${project.accent}`,
    ...
    <section id="work" style={{ position: 'relative', zIndex: 1, borderBottom: '1px solid var(--border)' }}>
      <div style={{ padding: '72px 48px 48px', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          ...
          <a href="https://github.com/jkifle" target="_blank" rel="noreferrer" className="btn-ghost">
            All on GitHub ↗
          </a>
        </div>
      </div>
